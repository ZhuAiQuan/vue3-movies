 ...
 * @LastEditTime: 2021-07-14 18:00:12
 * @LastEditors: zaq
 * @Reference: 
-->
<template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function onEnd() {
      ...
      if (moveFilm.offsetX > moveFilm.default) moveFilm.offsetX = moveFilm.default
      else if (moveFilm.offsetX < maxL) moveFilm.offsetX = maxL
    }
    function onMove(e: any) {
      const touch: any = e.targetTouches[0];
      const move = touch.pageX - moveFilm.left;// 移动宽度
      moveFilm.offsetX = moveFilm.offsetX + move;
      
      
       
      // moveFilm.endMove = touch.pageX
    }
    function onChecked(row: MoviesData, i: number) {
